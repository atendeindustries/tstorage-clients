...
def main() -> int:
    ...
    with Channel(
        host="127.0.0.1", port=2025, payload_type=TuplePayloadType("if?")
    ) as channel:  # "if?" corresponds to our payload(int, float, bool)
        ...
        for r in get_response.data:
            print(
                f"Cid: {r.key.cid}, Mid: {r.key.mid}, Moid: {r.key.moid}, Cap: {r.key.cap}, Acq: {r.key.acq}, PayloadType( int: {r.value[0]}, float: {r.value[1]}, bool: {r.value[2]}"
            )
    return 0
