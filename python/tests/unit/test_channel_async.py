...
async def send_data_mock(data: bytes) -> None:
    None


async def early_close_mock(response: ResponseAcq) -> ResponseAcq:
    return response
...
class BufferFeeder:
    ...
    async def feed(self, buffer: ReceiveBuffer, limit: int = 0, data: bytes = b"") -> int:
...
@pytest.fixture
def channel() -> AsyncChannel[int]:
    payload_type = StructPayloadType[int]("<i")
    return AsyncChannel(TSTORAGE_HOST, TSTORAGE_PORT, payload_type)


async def test_channel_connect_fail(channel: AsyncChannel[int]) -> None:
    assert not (await channel.connect()).is_ok()


async def test_channel_with_fail(channel: AsyncChannel[int]) -> None:
    with pytest.raises(OSError):
        async with channel:
            pass


async def test_channel_no_connect(channel: AsyncChannel[int]) -> None:
    assert not (await channel.get(Key.min(), Key.max())).is_ok()
    assert not (await channel.get_acq(Key.min(), Key.max())).is_ok()
    assert not (await channel.get_stream(Key.min(), Key.max(), lambda _: None)).is_ok()
    iter = channel.get_iter(Key.min(), Key.max())
    match e := await anext(iter):
        case ResponseAcq():
            assert not e.is_ok()
        case Record():
            raise RuntimeError("Impossible condition")
    assert not (await channel.put([])).is_ok()
    assert not (await channel.puta([])).is_ok()
...
async def test_channel_puts(
    ...
    feeder = BufferFeeder()
    monkeypatch.setattr(channel, "_writer", ())
    monkeypatch.setattr(channel, "_reader", ())
    monkeypatch.setattr(channel, "_send_data", send_data_mock)
    monkeypatch.setattr(channel, "_feed_buffer", functools.partial(feeder.feed, data=response))
    assert (await channel.put(records)) == expected
    assert (await channel.puta(records)) == expected
...
async def test_channel_get_acq(
    channel: AsyncChannel[int], response: bytes, expected: ResponseAcq, monkeypatch: pytest.MonkeyPatch
) -> None:
    feeder = BufferFeeder()
    monkeypatch.setattr(channel, "_writer", ())
    monkeypatch.setattr(channel, "_reader", ())
    monkeypatch.setattr(channel, "_send_data", send_data_mock)
    monkeypatch.setattr(channel, "_feed_buffer", functools.partial(feeder.feed, data=response))
    assert (await channel.get_acq(Key.min(), Key.max())) == expected
...
async def test_channel_get(
    ...
    assert (await channel.get(Key.min(), Key.max())) == expected
...
async def test_channel_get_stream(
    channel: AsyncChannel[int],
    response: bytes,
    ...
    assert (await channel.get_stream(Key.min(), Key.max(), collector.receive)) == expected
...
async def test_channel_get_iter(
    channel: AsyncChannel[int],
    response: bytes,
    expected: ResponseAcq,
    memory: int | None,
    ...
    monkeypatch.setattr(channel, "_early_close", early_close_mock)
    channel.memory_limit = memory
    results = [e async for e in channel.get_iter(Key.min(), Key.max())]
